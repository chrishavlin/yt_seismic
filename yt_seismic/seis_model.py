...
def geosphere2cart(lat,lon,radius):
    """
    seis_model.sgeosphere2cart(lat,lon,radius)
...
def cart2sphere(x,y,z,geo=True):
    ...
    if geo:
        lat = 90. - lat # equator is at 0, +90 is N pole
...
class netcdf(object):
    ...
    def setUsefulInfo(self):
        ...
        for key, value in self.data.dimensions.items():
            ...
            self.ranges[key]={
            'min':self.data.variables[key][:].min(),
            'max':self.data.variables[key][:].max(),
            }
            self.ranges[key]['mid']=(self.ranges[key]['min']+self.ranges[key]['max'])/2.
            self.domainCenter[key]=self.ranges[key]['mid']
